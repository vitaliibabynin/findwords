...
public class CordovaFacebook extends CordovaPlugin {
        ...
        m.registerCallback(callbackManager, new FacebookCallback<LoginResult>() {
            ...
            public void onSuccess(LoginResult loginResult) {
                ...
                if(a != null) {
                    ...
                    mappedResult.put("expirationDate", a.getExpires().getTime());
                }
    ...
    private void init(JSONArray args, final CallbackContext callbackContext){
        final AccessToken a = AccessToken.getCurrentAccessToken();
    ...
    private void invite(JSONArray args, final CallbackContext callbackContext){
        final int argc = args.length();
        ...
        final String appLinkUrl = args.optString(0);
        final String appInvitePreviewImageURL = args.optString(1);

        if (AppInviteDialog.canShow()) {
            callbackContext.error("Can not show AppInviteDialog");
        }
    ...
    private void share(JSONArray args, final CallbackContext callbackContext){
        ...
        final String shareLinkUrl = args.optString(0);
        final String shareTitle = args.optString(1);
        final String shareDescription = args.optString(2);
        ...
        shareDialog.registerCallback(this.callbackManager, new FacebookCallback<Sharer.Result>(){
            @Override
            public void onSuccess(Sharer.Result var1){
                Map<String, Object> mappedResult = new ArrayMap<String, Object>();

                if(null == var1.getPostId()){
                    callbackContext.error(new JSONObject(mappedResult));
                    return;
                }
            ...
            public void onError(FacebookException var1){
                callbackContext.error(null != var1 ? var1.getMessage() : "Facebook error");
            }
        });
        ...
        if (ShareDialog.canShow(ShareLinkContent.class)) {
            ...
            if(null != shareTitle){
                linkContent.setContentTitle(shareTitle);
            }
            if(null != shareDescription){
                linkContent.setContentDescription(shareDescription);
            }
    ...
    public boolean execute(String action, JSONArray args, final CallbackContext callbackContext) throws JSONException {
        if (EVENT.equals(action)) {
            ...
            this.invite(args, callbackContext);
